$(document).ready(function(){
	...
	{
		$('#bs-css').attr('href','/admin/css/bootstrap-'+theme_name+'.css');
	}
...
function docReady(){
        ...
        $(".country").change(function(){
           ...
           if($(this).val()!=""){
                $.ajax({         
                    ...
                    type:"post",
                    url: '/entitymanager/province/combo-load', 
        ...
        $(".input-file").livequery(function(){
            var $filename = $(this).parent().children(".filename");  
            var $fileurl       = $(this).parent().children(".fileurl");
            var $progress_bar  = $(this).parent().parent().children(".progress-bar");
            var $error_message = $(this).parent().parent().children(".error-input-message");
            var $download_icon = $(this).parent().children(".download-icon");
            
            if($filename.val()!=""){
                $(this).uniform({fileButtonHtml: 'Cambiar archivo',
                ...
                $(this).uniform({fileButtonHtml: 'Seleccionar archivo'});
            }
            
            $(this).liteUploader({
                    script: '/admin/upload/archivo?',
                    allowedFileTypes: "application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword,application/pdf,text/plain,application/vnd.ms-excel,application/vnd.ms-powerpoint",
                    maxSizeInBytes: 0,
                    customParams: {
                        'format': 'json',
        ...
        $(".input-image").livequery(function(){
            var $filename = $(this).parent().children(".filename");  
            var $fileurl       = $(this).parent().children(".fileurl");
            var $progress_bar  = $(this).parent().parent().children(".progress-bar");
            var $error_message = $(this).parent().parent().children(".error-input-message");
            var $thumbnail = $(this).parent().children(".thumbnail").children("a").children("img");
            var $image     = $(this).parent().children(".thumbnail").children("a");
            if($filename.val()!=""){
                $(this).uniform({fileButtonHtml: 'Cambiar imagen',
                                 fileDefaultHtml: $filename.val()}); 
            } else {
                $(this).uniform({fileButtonHtml: 'Seleccionar archivo'});
            }
	...
	//tabs
        $('#tabs').tabs({
                        beforeLoad: function(event, ui) {
                            $('#tabs').children("div").each(function(index, elem) {
                                //alert($(elem).attr("id") + " " + $(elem).is(':visible'));
                                if (!$(elem).is(':visible')){
	...
	//tooltip
	$('[rel="tooltip"],[data-rel="tooltip"]').livequery(function(){
            $(this).tooltip({"placement":"top",delay: { show: 400, hide: 200 }});
        })
	...
	/*var elf = $('.file-manager').elfinder({
		url : '/admin/misc/elfinder-connector/connector.php'  // connector URL (REQUIRED)
	}).elfinder('instance');*/

	//iOS / iPhone style toggle switch
	$('.iphone-toggle').livequery(function(){
            $(this).iphoneStyle();
        })
        
        
        // Errores
        $(".error-input-message").livequery(function(){
            var error = $(this).html();
     
            var hijo = $(this).prev("div").children();
            if($(hijo).attr("data-rel") == "chosen"){
                hijo = $(hijo).next();
            }
	...
	/*$('#file_upload').uploadify({
		'swf'      : '/admin/misc/uploadify.swf',
		'uploader' : '/admin/misc/uploadify.php'
	...
	//gallery colorbox
	$('.thumbnail a').livequery(function(){
            $(this).colorbox({rel:'thumbnail a', 
                              transition:"elastic", 
                              maxWidth:"95%", 
                              maxHeight:"95%"});
        });
