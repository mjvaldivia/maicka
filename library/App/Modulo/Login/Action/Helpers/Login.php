...
class Helper_Login extends Zend_Controller_Action_Helper_Abstract{
    ...
    public function index(){
        ...
        $this->_mensaje = "Ingrese su usuario y password";
	...
	if (!$request->isPost()) {
        ...
        if(strlen($user)==0 || strlen($password)==0){
           return $this->_setError("El usuario y password no puede ser vacio");
        }
    ...
    public function getUrlRetorno(){
        if(strlen($this->_request_url) == 0){
           $this->_request_url = "/" . $this->getRequest()->getModuleName() . "/index";
        }
